...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__popup_themes__ = __webpack_require__(5);
...
((() => {
	...
	function loadTheme(items, isDefault, active, ison) {
			for (const id in css_rules) {
					setColor(id, items[id]);
			}
	}

	function getImg() {
			return __WEBPACK_IMPORTED_MODULE_0_jquery___default()(new Image());
	}
	...
	function setColor(e_id, c) {
			...
			if (c && c != "false") {
					let rule_t = rule.replace(/#color#/g, c);
	...
	return {
			initialize() {
					chrome.storage.sync.get({
							...
							loadTheme(items["themes"][items["active_theme"]] || __WEBPACK_IMPORTED_MODULE_1__popup_themes__["a" /* default_themes */][items["active_theme"]] || {}, !items["themes"][items["active_theme"]], items["active_theme"], +items["isOn"]);
					});
