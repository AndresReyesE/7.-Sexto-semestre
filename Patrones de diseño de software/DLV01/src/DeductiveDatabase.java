...
// --> Add External JARS and then add the DLV Wrapper jar
...
public class DeductiveDatabase 
{
	...
	private Vector<String> newRulesList;	
	...
	// the path of the facts and rule files in DLV
	private String factsFilePath;
	...
	public DeductiveDatabase(String aDlvPath)
	{
		...
		newRulesList = new Vector<String>();		
		factsToRemoveList = new Vector<String>();	
	}//end constructor
	
	
	// to indicate a file where we have facts to include
	public void setFacts(String aFactsFilePath)
	{
		inputProgram.addFile(aFactsFilePath);
		factsFilePath = aFactsFilePath;
	}//end setFacts
	
	// to indicate a file where we have rules to include
	public void setRules(String aRulesFilePath)
	{
		inputProgram.addFile(aRulesFilePath);
		rulesFilePath = aRulesFilePath;
	}//end setFacts
	...
	public void addFact(String aFact)
	{
		...
		factString = aFact + ".";
		newExpression = new Rule(factString);
		inputProgram.addExpression(newExpression);
		answerSet = getAnswerSet();	
		if(!newFactsList.contains(aFact))
			newFactsList.add(aFact);
		//end if
	}//end addFact
	...
	// given a String
	public void removeFact(String aFact)
	{
		Rule anExpression;
		String factString;
		//------------------
		factString = aFact + ".";
		anExpression = new Rule(factString);
		inputProgram.removeExpression(anExpression);
		answerSet = getAnswerSet();	
		if(newFactsList.contains(aFact))
		{
			newFactsList.remove(aFact);
		}//end if
		factsToRemoveList.add(aFact);
	}//end removeFact
